...
async function populateDetailsAndLoadPoster(anime) {
    ...
    if (nextEpisodeContainer && nextEpisodeDetails) {
    ...
    // Данные уже должны быть в объекте 'anime', если API их предоставляет.
    // Если нет, соответствующие секции можно скрыть.
    // В данном случае, Shikimori API не возвращает related/similar в одном запросе.
    // Эти функции нужно будет либо удалить, либо переделать на отдельные запросы,
    // но для исправления текущей ошибки (многократные вызовы) мы их пока упростим.
    // loadRelatedAnime(anime);  // TODO: Implement properly or remove
    // loadSimilarAnime(anime); // TODO: Implement properly or remove
