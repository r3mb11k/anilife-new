...
async function loadAnimeDetails(animeId) {
    ...
    // Start fetching related & similar IMMEDIATELY (do not await yet)
    const relatedPromise = fetchFromApi('get_related', { id: animeId });
    const similarPromise = fetchFromApi('get_similar', { id: animeId });

    // Show skeletons for side lists while data is loading
    const showSkeletons = (container, count = 10) => {
        if (!container) return;
    ...
    showSkeletons(document.getElementById('related-anime-list'));
    showSkeletons(document.getElementById('similar-anime-list'));

    // Показать контент сразу (со скелетонами), скрыть прелоадер
    ...
    try {
        ...
        // Дожидаемся параллельных запросов и рендерим списки
        const [relatedSettled, similarSettled] = await Promise.allSettled([relatedPromise, similarPromise]);

        if (relatedSettled.status === 'fulfilled') {
            renderRelatedAnimeList(relatedSettled.value);
        }
        if (similarSettled.status === 'fulfilled') {
            renderSimilarAnimeList(similarSettled.value);
        }
        ...
        // nothing extra here, контент уже отображается
    }
}
...
function renderSimilarAnimeList(rawSimilar) {
